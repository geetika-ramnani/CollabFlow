...
import { loadBoard, removeTask, saveTask } from '../store/board.slice'
...
export function TaskDetails() {
	...
	useEffect(() => {
		if (!board) dispatch(loadBoard(boardId))
	...
	async function onRemoveTask() {
		try {
			console.log('remove:')
			const action = await dispatch(removeTask({ taskId, groupId, boardId }))
	...
	async function onCopyTask() {
		...
		copyTask.id = null
		try {
			await dispatch(saveTask({ task: copyTask, groupId, boardId }))
		} catch (err) {
	...
	function onSaveTask(ev, updateTask) {
		ev.preventDefault()
		setTask(updateTask)
		dispatch(saveTask({ task: updateTask, groupId, boardId }))
	}
