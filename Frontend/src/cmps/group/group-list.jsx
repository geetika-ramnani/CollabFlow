...
import { removeGroup, saveGroup, updateBoard } from '../../store/board.slice'
...
export function GroupList() {
	...
	async function onSaveNewGroup(ev) {
		...
		try {
			await dispatch(saveGroup({ group: groupToEdit, boardId: board._id }))
			closeAddNewGroup()
			setGroupToEdit(boardService.getEmptyGroup())
		} catch (err) {
	...
	async function handleEditGroup({ target }) {
		...
		try {
			await dispatch(saveGroup({ group: currGroup, boardId: board._id }))
		} catch (err) {
	...
	async function onRemoveGroup(groupId) {
		try {
			await dispatch(removeGroup({ groupId, boardId: board._id }))
		} catch (err) {
	...
	async function onCopyGroup(group) {
		...
		try {
			await dispatch(saveGroup({ group: duplicatedGroup, boardId: board._id }))
		} catch (err) {
